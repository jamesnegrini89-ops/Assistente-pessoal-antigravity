...
export async function renderAgora(root,ctx){
  const [memories,decisions,modules,name,mode]=await Promise.all([getAll('memories'),getAll('decisions'),getAll('forgeModules'),getSetting('profileName','VIP'),getSetting('currentMode','Pessoal')]);
  ...
  root.innerHTML=`<section class="page">
    <div class="hero">
      ...
      <div class="hero-meta"><span class="chip"><i></i> Privado no aparelho</span><span class="chip">${memories.length} memórias</span><span class="chip">${openDecisions.length} decisões abertas</span><span class="chip">${modules.length} módulos da FORJA</span></div>
    ...
    <div class="grid grid-4" style="margin-top:16px">
      ...
      <article class="card stat-card"><span class="stat-label">MODO ATUAL</span><strong class="stat-value" style="font-size:1.15rem">${escapeHTML(mode)}</strong><small class="stat-label">contexto selecionado</small></article>
    </div>
  </section>`;
