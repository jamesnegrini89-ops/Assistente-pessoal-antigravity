...
const renderers={agora:renderAgora,qg:renderQG,finance:renderFinanceiro,memoria:renderMemoria,oraculo:renderOraculo,forja:renderForja,guia:renderGuia,guardiao:renderGuardiao,configuracoes:renderConfiguracoes};
let settings={};
...
function buildNavigation(){
  const desktop=['agora','qg','finance','memoria','oraculo','forja','guia','guardiao','configuracoes'];
...
function showOnboarding(){
  ...
  const render=()=>{
    ...
    if(step===0)body.innerHTML=`<span class="eyebrow">ASSISTENTE ${APP_VERSION}</span><h2>Bem-vindo ao Assistente Clean</h2><p>Esta instalação começará vazia e armazenará seus dados neste navegador. A estrutura inicial inclui AGORA, QG, GÊMEO FINANCEIRO, MEMÓRIA, ORÁCULO, FORJA, VIP GUIA, GUARDIÃO e CONFIGURAÇÕES.</p><div class="grid grid-2"><div class="kv"><small>FUNCIONAMENTO</small><strong>Offline-first</strong></div><div class="kv"><small>IDENTIDADE</small><strong>Clean Tech (Ice Gray e Azul)</strong></div></div><div class="setup-actions"><span></span><button id="setupNext" class="btn btn-primary">Começar</button></div>`;
